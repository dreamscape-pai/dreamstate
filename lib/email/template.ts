...
export async function generateTicketConfirmationEmail(
  ...
  let subject: string;
  if (ticketCount === 1) {
    subject = `Dreamstate Ticket: You are ${data.tickets[0].faction.displayName}!`;
  } else {
    subject = `${ticketCount} Dreamstate Tickets`;
  }

  // Generate ticket sections
  const ticketSections = data.tickets.map((ticket, index) => `
    <!-- Ticket ${index + 1} -->
    ${index > 0 ? '<div style="height: 40px; border-top: 2px solid #2d3748; margin: 40px 0;"></div>' : ''}

    <!-- Faction Assignment -->
    <div style="background-color: rgba(107, 70, 193, 0.2); border-left: 4px solid ${ticket.faction.colorToken}; padding: 20px; margin-bottom: 30px; border-radius: 8px;">
      <h2 style="margin: 0 0 10px 0; color: #c4b5fd; font-size: 24px;">
        Ticket #${ticket.ticketNumber}: You are ${ticket.faction.displayName}
      </h2>
    ...
    <div style="text-align: center; margin: 30px 0;">
      <h3 style="color: #c4b5fd; margin-bottom: 20px;">Ticket #${ticket.ticketNumber} QR Code</h3>
      <div style="background: white; display: inline-block; padding: 20px; border-radius: 12px;">
        <img src="cid:qrcode-${ticket.ticketNumber}" alt="Ticket QR Code" style="display: block; width: 300px; height: 300px;" />
      </div>
      <p style="margin: 15px 0 0 0; color: #9ca3af; font-size: 14px;">
        Ticket #${ticket.ticketNumber}
      </p>
      <p style="margin: 10px 0 0 0; font-size: 14px;">
        <a href="${data.siteUrl}/verify/${ticket.verificationToken}" style="color: #8b5cf6; text-decoration: none;">
...
<head>
  ...
  <title>Your Dreamstate ${ticketCount === 1 ? 'Ticket' : 'Tickets'}</title>
</head>
        ...
        <table role="presentation" style="width: 600px; max-width: 100%; background-color: #1a1a2e; border-radius: 12px; overflow: hidden;">
          ...
          <tr>
            <td style="background: linear-gradient(135deg, #6b46c1 0%, #4c1d95 100%); padding: 40px; text-align: center;">
              ...
              ${ticketCount > 1 ? `<p style="margin: 10px 0 0 0; color: #e9d5ff; font-size: 18px; font-weight: bold;">${ticketCount} Tickets</p>` : ''}
            </td>
          </tr>
          ...
          <tr>
            <td style="padding: 40px;">
              ...
              <div style="background-color: #16213e; border-radius: 8px; padding: 25px; margin-top: 40px;">
                ...
                <ul style="margin: 0; padding-left: 20px; color: #e6e9f0; line-height: 1.8;">
                  <li>Present ${ticketCount === 1 ? 'this' : 'these'} QR code${ticketCount > 1 ? 's' : ''} at the door for entry verification</li>
                  <li>Bring this email (digital or printed) with you</li>
                  <li>Arrive ready to experience faction-specific performances</li>
                  <li>Your faction assignment${ticketCount > 1 ? 's are' : ' is'} final and cannot be changed</li>
                </ul>
              </div>
              ...
              <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #2d3748;">
                <p style="color: #9ca3af; font-size: 14px; line-height: 1.6; margin: 0;">
                  ...
                  <strong style="color: #c4b5fd;">Ticket${ticketCount > 1 ? 's' : ''}:</strong> ${data.tickets.map(t => `#${t.ticketNumber}`).join(', ')}
                </p>
              </div>
